...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	if summary.ImageInputTokens != 0 {
		...
		other.SetPublic("image_input_tokens", summary.ImageInputTokens)
		// Keep the historical key for existing log readers. Despite its name,
		// it has always contained image input tokens.
		other.SetPublic("image_output", summary.ImageInputTokens)
	}
	if summary.ImageOutputTokens != 0 {
		...
		if summary.ImageOutputRatioSet {
			other.SetPublic("image_output_ratio", summary.ImageOutputRatio)
		}
	}
