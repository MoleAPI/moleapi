...
function DetailPreview(props: { other: LogOtherData; isAdmin: boolean }) {
  ...
  const table = useReactTable({
    ...
    columns: useCommonLogsColumns(props.isAdmin),
...
test.each([
  ...
  {
    name: 'missing price retains provider information',
    other: {},
    expected: 'Plugin',
  },
])('$name stays visible in the compact preview', ({ other, expected }) => {
...
test('quota saturation remains first alongside plugin and billing details', () => {
  const preview = renderPreview({
    model_price: 0.25,
    admin_info: {
      task_plugin: plugin,
  ...
  expect(preview.textContent).toMatch(/^Quota clamped/)
