...
test('first install shows source on demand and disables repeat submission until success', async () => {
  ...
  const code = screen.getByRole('tabpanel', { name: 'Full source' })
  expect(code).toHaveTextContent('const')
  expect(code).toHaveTextContent('"demo"')
  expect(code).toHaveTextContent('// comment')
  expect(code.querySelector('[contenteditable="true"]')).toBeNull()
