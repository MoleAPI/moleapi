...
export function Hero(props: HeroProps) {
  ...
  return (
    <section className='relative z-10 overflow-hidden px-6 pt-24 pb-16 md:pt-32 md:pb-24 lg:pt-36 lg:pb-28'>
            ...
            <div className='flex flex-wrap items-center gap-3'>
              {SUPPORTED_APPS.map((app) => (
                <a
                  ...
                  className='group border-border/40 bg-muted/15 text-foreground/80 hover:border-border hover:bg-muted/30 hover:text-foreground flex items-center gap-2.5 rounded-full border px-4 py-2 text-sm font-medium shadow-[0_1px_2.5px_rgba(0,0,0,0.01)] backdrop-blur-xs transition-[color,background-color,border-color,transform] duration-200 hover:-translate-y-px'
                >
      ...
      <div
        className='landing-animate-fade-up mx-auto mt-16 max-w-6xl opacity-0 md:mt-20'
      ...
      >
        ...
        <div className='flex flex-wrap items-center justify-center gap-2.5'>
          {MODEL_PROVIDERS.map((provider) => (
            <div
              key={provider.name}
              className='border-border/40 bg-muted/15 text-foreground/70 flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-medium'
            >
              <span aria-hidden='true' className='flex shrink-0'>
                {provider.icon}
              </span>
          ...
          <div className='border-border/40 bg-muted/15 text-muted-foreground flex items-center rounded-full border px-3 py-1.5 text-xs font-semibold tabular-nums'>
            30+
          </div>
        </div>
      </div>
