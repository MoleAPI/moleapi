...
func (mi *Model) Update() error {
	return metadataTransaction(func(tx *gorm.DB) error {
		...
		return tx.Model(&Model{}).Where("id = ?", mi.Id).
			Select("model_name", "description", "description_i18n", "icon", "tags", "vendor_id", "endpoints", "status", "sync_official", "name_rule", "updated_time").Updates(mi).Error
	})
}
...
func GetBoundChannelsByModelsMap(modelNames []string) (map[string][]BoundChannel, error) {
	...
	err := DB.Table("channels").Select("abilities.model as model, channels.name as name, channels.type as type").
		Joins("JOIN abilities ON abilities.channel_id = channels.id").
		Where("abilities.model IN ? AND abilities.enabled = ?", modelNames, true).Distinct().Scan(&rows).Error
	if err != nil {
		return nil, err
	}
	for _, row := range rows {
		result[row.Model] = append(result[row.Model], BoundChannel{Name: row.Name, Type: row.Type})
	}
	return result, nil
}
