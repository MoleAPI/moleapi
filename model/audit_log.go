...
// RecordOperationAuditLog records an operation/security event in the
// independent audit table while retaining the local legacy log writer for
// older, explicitly migrated call sites.
func RecordOperationAuditLog(logUserId, actorRole int, content, ip, action string, params map[string]any, adminInfo *AuditAdminInfo, auditInfo *AuditRequestInfo, request ...*gin.Context) {
	if actorRole == 0 && adminInfo != nil {
		actorRole = adminInfo.AdminRole
	}
	entry := AuditLog{
		UserId: logUserId, ActorRole: actorRole, Content: content, Ip: ip,
		Action: action, Success: true,
		Other: AuditOther{Op: &AuditOperation{Action: action, Params: params}, AdminInfo: adminInfo, AuditInfo: auditInfo},
	}
	if adminInfo == nil {
		entry.Category = AuditCategorySecurity
	} else {
		entry.Category = AuditCategoryOperation
	}
	if auditInfo != nil {
		entry.Status = auditInfo.Status
		entry.Success = auditInfo.Success
		entry.Method = auditInfo.Method
		entry.Route = auditInfo.Route
	}
	var c *gin.Context
	if len(request) > 0 {
		c = request[0]
...
// RecordLoginLog writes successful login events to the independent audit
// table. The caller already has the user object, so no extra user lookup is
// needed for the common login path.
func RecordLoginLog(userId, actorRole int, username, content, ip, action string, params map[string]any, other AuditOther, request ...*gin.Context) {
