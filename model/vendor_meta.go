...
func GetVendorByID(id int) (*Vendor, error) {
	if id < 0 {
		for name, displayID := range defaultVendorDisplayIDs {
			if displayID == id {
				v := &Vendor{Id: id, Name: name, Icon: getDefaultVendorIcon(name), Status: 1}
		...
		return nil, fmt.Errorf("vendor %d not found", id)
	}
	var v Vendor
