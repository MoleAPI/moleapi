...
func modelManagementDB(t *testing.T, kind, dsn string) *gorm.DB {
	...
	restoreRatios := []struct {
		...
		{ratio_setting.ImageOutputRatio2JSONString(), ratio_setting.UpdateImageOutputRatioByJSONString},
	...
	for _, dialect := range []struct{ kind, env string }{{"sqlite", ""}, {"mysql", "TEST_MYSQL_DSN"}, {"postgres", "TEST_POSTGRES_DSN"}} {
		t.Run(dialect.kind, func(t *testing.T) {
			...
			for _, tc := range []struct {
				...
				{"conversion-image-output", model.PricingValues{"ModelRatio": float64(2), "CompletionRatio": float64(3), "ImageOutputRatio": float64(4)}, `tier("base", p * 4 + c * 12 + img_o * 16)`, ""},
				{"conversion-image-output-free", model.PricingValues{"ModelRatio": float64(2), "CompletionRatio": float64(3), "ImageOutputRatio": float64(0)}, `tier("base", p * 4 + c * 12 + img_o * 0)`, ""},
