...
func TestResponsesWSChannelRoutingRequiresExplicitOptIn(t *testing.T) {
	for _, channelType := range []int{constant.ChannelTypeOpenAI, constant.ChannelTypeCodex, constant.ChannelTypeSub2API, constant.ChannelTypeNewAPI, constant.ChannelTypeAdvancedCustom, constant.ChannelTypeAnthropic} {
		for _, converter := range []string{"", "none", "openai_responses_to_openai_chat"} {
			channel := &model.Channel{Type: channelType}
			channel.SetSetting(dto.ChannelSettings{ResponsesWebSocketEnabled: true})
			channel.SetOtherSettings(dto.ChannelOtherSettings{AdvancedCustom: &dto.AdvancedCustomConfig{Routes: []dto.AdvancedCustomRoute{{IncomingPath: "/v1/responses", Converter: converter}}}})
			ok, _ := model.ChannelSatisfiesFilters(channel, "ws-model", []appdto.ChannelFilter{{Kind: appdto.FilterResponsesWebSocket}})
			assert.Equal(t, channelType != constant.ChannelTypeAnthropic && (channelType != constant.ChannelTypeAdvancedCustom || converter != "openai_responses_to_openai_chat"), ok, "type=%d converter=%s", channelType, converter)
