...
var codingPlanPresets = []CodingPlanPreset{
	...
	{
		ID:               CodingPlanProviderQwenCoding,
	...
	{
		ID:               CodingPlanProviderQwenTokenPlan,
		...
		ResponsesBaseURL: "https://token-plan.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1",
		ModelListBaseURL: "https://token-plan.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1",
	},
	{
		ID:               CodingPlanProviderMiniMax,
	...
	{
		ID:               CodingPlanProviderOpenCodeGo,
...
func ResolveCodingPlanPreset(value string) (CodingPlanPreset, bool) {
	...
	for _, preset := range codingPlanPresets {
		if strings.EqualFold(value, preset.ID) ||
			strings.EqualFold(value, normalizeCodingPlanPresetValue(preset.OpenAIBaseURL)) ||
			strings.EqualFold(value, normalizeCodingPlanPresetValue(preset.AnthropicBaseURL)) ||
			strings.EqualFold(value, normalizeCodingPlanPresetValue(preset.ResponsesBaseURL)) {
			return preset, true
		}
	}
	return CodingPlanPreset{}, false
...
func codingPlanRoute(incomingPath string, upstreamPath string, converter string) AdvancedCustomRoute {
...
func codingPlanJoinPath(baseURL string, path string) string {
