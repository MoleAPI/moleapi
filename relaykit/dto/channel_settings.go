...
type ChannelOtherSettings struct {
	...
	ChannelProbeModels                    []string              `json:"channel_probe_models,omitempty"`
...
// IsNative excludes protocol conversion from persistent Responses connections.
...
func (c *AdvancedCustomConfig) SupportedEndpointTypesForModel(model string) []types.EndpointType {
	...
	model = strings.TrimSpace(model)
	endpoints := make([]types.EndpointType, 0, len(c.Routes)+2)
	seen := make(map[types.EndpointType]struct{}, len(c.Routes))
	addEndpoint := func(endpointType types.EndpointType) {
...
func inferredAdvancedCustomTextEndpointTypes(route AdvancedCustomRoute) []types.EndpointType {
	upstreamEndpoint, ok := advancedCustomRouteUpstreamTextEndpointType(route)
	if !ok {
		return nil
	}
	endpoints := make([]types.EndpointType, 0, 3)
	for _, incomingEndpoint := range []types.EndpointType{
...
func advancedCustomRouteUpstreamTextEndpointType(route AdvancedCustomRoute) (types.EndpointType, bool) {
...
func advancedCustomTextEndpointType(endpointType types.EndpointType) (types.EndpointType, bool) {
	switch endpointType {
...
func advancedCustomTextConverter(incomingEndpoint types.EndpointType, upstreamEndpoint types.EndpointType) (string, bool) {
	...
	case types.EndpointTypeOpenAI:
		...
		case types.EndpointTypeOpenAIResponse:
