...
type Registry struct {
	...
	// OverrideEnabled preserves the local safety switch for administrator-supplied
	// plugins; upstream currently assumes this layer is always active.
...
func (r *Registry) register(source string, options Options, factory bool) (*LoadedPlugin, error) {
	...
	routingOverrides := overridePlugins
	if !r.overrideEnabled.Load() {
		routingOverrides = map[string]*LoadedPlugin{}
	}
	generation, routingErrors, err := r.prepareGeneration(filterDisabledFactory(factoryPlugins, r.disabledFactory), routingOverrides, false, nil)
	if err != nil {
	...
	r.publishGeneration(generation, routingErrors, r.resolveActiveOverrides(generation, routingOverrides))
	return plugin, nil
}
...
func (r *Registry) SetEnabled(enabled bool) {
	...
	routingOverrides := r.routingOverrides()
	generation, routingErrors, err := r.prepareGeneration(filterDisabledFactory(r.factory, r.disabledFactory), routingOverrides, true, retainCurrent)
	if err != nil {
...
// SetOverrideEnabled toggles only the administrator override layer. Factory
// plugins and their disabled-key policy remain unchanged.
func (r *Registry) SetOverrideEnabled(enabled bool) {
	...
	routingOverrides := r.routingOverrides()
	retainCurrent := map[string]struct{}(nil)
	if enabled {
		retainCurrent = pluginMapKeys(r.override)
	}
	generation, routingErrors, err := r.prepareGeneration(filterDisabledFactory(r.factory, r.disabledFactory), routingOverrides, true, retainCurrent)
...
func (r *Registry) routingOverrides() map[string]*LoadedPlugin {
	if r.overrideEnabled.Load() {
		return r.override
	}
...
func (r *Registry) SetDisabledFactoryKeys(keys []string) {
	...
	retainCurrent := pluginMapKeys(r.override)
	routingOverrides := r.routingOverrides()
	generation, routingErrors, err := r.prepareGeneration(filterDisabledFactory(r.factory, next), routingOverrides, true, retainCurrent)
	if err != nil {
		r.recordRebuildFailure(err)
		return
	}
